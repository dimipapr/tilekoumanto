...
## 11. MQTT message contract

All MQTT messages use a common JSON envelope.

The envelope is shared by both communication directions:

- field device to backend
- backend to field device

The envelope is generic. The `payload` object is topic-specific and must be validated according to the MQTT topic that received the message.

All fields defined in a message contract are required unless a contract explicitly says otherwise.

For the current MVP, only field-device-to-backend telemetry is used. Backend-to-field-device messages may use the same envelope later, but remote control is not part of the MVP.
...
| Field                | Type   | Meaning                                                            |
| -------------------- | ------ | ------------------------------------------------------------------ |
| `meta`               | object | Message metadata independent of topic and direction                |
| `meta.unix_time_ms`  | integer | Message production time                                            |
| `payload`            | object | Topic-specific message body                                        |

The receiver stores its own receive timestamp separately from `meta.unix_time_ms`.

## Pump telemetry message
...
| ------------- | ------ | ---------------------------------------- |
| `device_uuid` | string | Stable UUID identifying the field device |

### Payload schema

```json
"payload":{
    "mains_power_present":"boolean",
...
| Field                 | Type    | Meaning                                                                               |
| --------------------- | ------- | -------------------------- |
| `mains_power_present` | boolean | Detected mains power state |
| `pump_relay_active`   | boolean | Detected pump relay state  |
...
When a valid pump telemetry message is received, the backend stores the reported state and updates the latest known state for the device.

Invalid messages must not update the latest known state.